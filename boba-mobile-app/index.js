...
// PASTE YOUR REAL ANON KEY BELOW INSIDE QUOTES:
...
function App() {
  const [activeTab, setActiveTab] = useState('chat'); // 'chat' | 'map' | 'merchant'
  ...
  const sendMessage = async () => {
    ...
    setTimeout(async () => {
      ...
      if (pList && pList.length > 0) {
        matchingProducts = pList.filter(p =>
      ...
      if (matchingProducts.length > 0) {
        ...
        const storeList = bList ? bList.map(b => `📍 ${b.branch_name}: ${b.address || 'Address N/A'} (${b.phone || 'N/A'})`).join('\n') : 'No store branches found.';
        replyText = `Here are our physical store branches:\n\n${storeList}`;
      } else {
        const allItems = pList ? pList.map(p => `• ${p.title} - $${p.price} (Stock: ${p.stock_quantity})`).join('\n') : 'No items listed.';
...
// Registers this file as the root entry point for Expo Go
